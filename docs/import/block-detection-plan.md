...
1. **Page selection, not detection.** On one of the three sites, five of the six pages chosen from the sitemap were image-attachment pages generated by its publishing system rather than articles. The blocks harness read them correctly; they were the wrong pages to read. Every weak number for that site comes from this. Whichever component chooses pages to import needs its own evaluation.
...
4. **Two small ordering defects:** one page has no navigation bar, and on two pages a block sits above the navigation bar.
