...
1. **Page selection, not detection.** On one of the three sites, five of the six pages chosen from the sitemap were image-attachment pages generated by its publishing system rather than articles. The blocks harness read them correctly; they were the wrong pages to read. Every weak number for that site comes from this.

   Partly fixed (see section 5d). **The root cause is still open:** when an import is capped at N pages, the picker takes the alphabetically first N within each depth, so any site whose sitemap begins with machine-named pages yields machine-named pages.
...
## 5d. Page selection: what was fixed, and what is still wrong (22 September 2026)

Two real defects in `lib/studio/import/services/sitemap-discovery.service.ts` are fixed:

- `isAssetUrl` read the extension from the last path segment, and the last segment of a path ending in `/` is empty, so it returned false for every trailing-slash address: `/photo.jpg/` and `/doc.pdf/` were both imported as pages. It now strips trailing slashes.
- A new `isLikelyAttachmentPageUrl` recognises a final segment ending in `-jpg`, `-png` and the rest, which is how WordPress rewrites `name.jpg` into a page of its own. Applied at all eight places the service already checked for assets. It is a heuristic and says so: a legitimate page whose address ends in `-jpg` would be rejected.
- `compareEntries` now uses the sitemap's own `priority` when both entries publish one and they differ, before falling back to depth and alphabet. When no priority is published, or they are all equal, the order is exactly what it was.

**A correction to what section 5b first claimed:** ignoring `priority` was named as a cause. It is a real defect, but it is not this one. The site in question **publishes no `priority` at all**, and on another test site five of six pages share `priority` 0.9, so priority ordering could not have changed either result.

**The root cause is not fixed.** Re-importing the same site afterwards replaced the five attachment pages with five more machine-named figure pages (`/08-01/`, `/08-02/`, `/08-03a/` ... all titled "Figure It Out"), because they do not end in `-jpg` and still sort first. The picker takes the alphabetically first N pages within each depth, so any site whose sitemap begins with machine-named entries gives machine-named entries. Narrowing the heuristic further would be a game of naming conventions.

Options for a real fix, needing a decision rather than more patching:

1. **Prefer pages the site links to from its own navigation.** The service already crawls links, so the signal is at hand; a page in the main menu is a page the site considers real. Most principled, most work.
2. **Spread the selection across the sitemap** instead of taking a prefix, so a capped run samples the site rather than its first pages. Cheap and robust to any ordering pathology; loses "most important first" on sites where order is meaningful.
3. **Rank slugs by how word-like they are**, pushing numeric and short-code slugs down. Cheap, but another heuristic to maintain.
