...
const scored=(score:Score)=>score.rows.filter(row=>!row.ignored)
...
function accuracy(pages:PageScores[],runs:number[]) {
  ...
  if(values.some(value=>!value.total))throw new Error('Cannot calculate accuracy with zero scored blocks')
  return values.reduce((sum,value)=>sum+value.correct/value.total,0)/values.length
}
...
function diagnostics(pages:PageScores[],familySet?:string) {
  ...
  const cleanCount=pages.filter(page=>page.scores.every(score=>scored(score).every(row=>row.verdict==='correct'))).length
...
export async function generateAccuracy(root:string,options:AccuracyOptions) {
  ...
  const manifest=validatePages(await readJson(path.join(root,'pages.json'))),pages:PageScores[]=[]
  for(const page of Object.keys(manifest).sort()){
    ...
    for(const run of options.runs){
      ...
      let score:Score
      try{score=await readJson(file)}catch(error){if((error as NodeJS.ErrnoException).code==='ENOENT')throw new Error(`Missing stick score for page ${page}, run ${run}`);throw error}
  ...
  const result={arm:options.arm,runs:options.runs,familySet:options.familySet||null,
    development:{accuracy:devAccuracy,interval:devInterval,pages:development.length,correctBlocks:sum(devCounts.map(v=>v.correct)),...diagnostic},
    heldOut:{accuracy:heldAccuracy,interval:heldInterval,pages:heldOut.length,scoredBlocks:sum(heldCounts.map(v=>v.total))},noise}
  ...
  const groupSize=Math.max(1,Math.ceil(groups.length/12))
