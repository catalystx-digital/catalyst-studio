...
test('near twin becomes right; other family stays wrong; checks and ignored blocks stay unchanged',()=>{
  ...
  expect(normal.rows[0].verdict).toBe('wrong type');expect(grouped.rows[0].verdict).toBe('correct');expect(grouped.rows[0].checks).toEqual(normal.rows[0].checks)
  ...
  for(const components of [[],[output]])expect(scoreSheet(ignored,components,url,{families:family()}).rows).toEqual(scoreSheet(ignored,components,url).rows)
  const multiple=sheet([entry({label:label({containsMultipleComponents:true,componentTypes:['hero','hero-split']})})])
  expect(scoreSheet(multiple,[component,component],url,{families:family()}).rows[0].verdict).toBe('correct')
...
test('CLI validates paired flags and passes them to family arm plans and isolated score paths',async()=>{
  expect(()=>parseEval(['score','--family-set','A'])).toThrow('together');expect(()=>parseEval(['score','--families','families.json','--family-set','C'])).toThrow('A or B')
  expect(()=>parseEval(['arms','--arms','blocks-production','--run','r1','--families','families.json','--family-set','A'])).toThrow('only to jev-pick')
  expect(()=>scoreDirectory('garden',{families:'families.json',familySet:'A',ignoreItemCount:true})).toThrow('unchanged')
