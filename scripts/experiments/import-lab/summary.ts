import { familyOutputRoot, scoreDirectory, type FamilySet } from './families'
...
export async function readSavedResults(families?:FamilySet,comparisonBaseline=false): Promise<SavedResult[]> {
  ...
  for (const page of pages) {
    ...
    const scores:any[]=[],scoreRoot=families?scoreDirectory(page,{families:'loaded',familySet:families.set}):path.join(dataRoot(),'labels',page,'scores')
    for (const file of await jsonFiles(scoreRoot)) {const score=await readJson(path.join(scoreRoot,file));if(file!=='score-runs.json')scores.push(score)}
    ...
    for (const run of await directories(path.join(dataRoot(),'runs',page))) {
      ...
      for (const name of names) {
        ...
        await add(name==='off'||name==='pre-repair'?'today-off':name==='on-own-page'?'today-on-own-page':'today-'+name,run,directory,scores.find(s=>s.run===run&&s.arm===name),record,true)
      }
    }
    for (const arm of await directories(path.join(dataRoot(),'arms',page))) for (const run of await directories(path.join(dataRoot(),'arms',page,arm))) {
      ...
      await add(arm,run,directory,scores.find(s=>s.name===arm+'--'+run || (s.arm===arm&&s.run===run)),await optionalJson(path.join(directory,'run.json')))
    }
  }
  return results
}
