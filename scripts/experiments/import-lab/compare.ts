...
export function buildComparison(pages:ComparisonPage[],heldOut:{baseline:number;candidate:number},options:CompareOptions){
  ...
  const report=[
    `${decision}: the 15 section families import ${tenth(candidate)} in 10 sections correctly, against ${tenth(baseline)} in 10 today (development pages).`,
    ...
    `Held-out pages: families ${tenth(heldOut.candidate)} in 10, today ${tenth(heldOut.baseline)} in 10.`,
    '---',
    ...
    `Development today: ${pct(baseline)} (${baseTotals.correct}/${baseTotals.total}); 95% interval ${pct(baselineInterval[0])} to ${pct(baselineInterval[1])}.`,
    `Development families: ${pct(candidate)} (${candTotals.correct}/${candTotals.total}); 95% interval ${pct(candidateInterval[0])} to ${pct(candidateInterval[1])}.`,
    ...
    ...CHECKS.map(check=>`${check} failure: today ${pct(checkResults[check].baseline)} (${checkResults[check].baselineCounts.failed}/${checkResults[check].baselineCounts.total}), families ${pct(checkResults[check].candidate)} (${checkResults[check].candidateCounts.failed}/${checkResults[check].candidateCounts.total}), paired change ${point(checkResults[check].change)} points (95% ${point(checkResults[check].interval[0])} to ${point(checkResults[check].interval[1])}).`),
    `Sections dropped after validation: today ${count(baseTotals.dropped,baseTotals.blocks)}, families ${count(candTotals.dropped,candTotals.blocks)}.`,
    `Extra and junk: today ${baseTotals.extra}+${baseTotals.junk} (${pct(ratio(baseTotals.extra+baseTotals.junk,baseTotals.total))}), families ${candTotals.extra}+${candTotals.junk} (${pct(ratio(candTotals.extra+candTotals.junk,candTotals.total))}); paired rise ${point(extraGain)} points; noise band ${point(noise)} points (single-run, conservative).`,
    `Cost and wall-clock: today ${baseTotals.cost===null?'unknown':'$'+baseTotals.cost.toFixed(4)} / ${baseTotals.seconds===null?'unknown':baseTotals.seconds.toFixed(1)+'s'}, families ${candTotals.cost===null?'unknown':'$'+candTotals.cost.toFixed(4)} / ${candTotals.seconds===null?'unknown':candTotals.seconds.toFixed(1)+'s'} (${pages.length*runs.length} page-runs per arm).`,
...
async function loadRun(root:string,page:string,arm:string,run:string,familySet:string):Promise<ComparisonRun>{
  ...
  if(!await optional<any>(scoreFile))await scorePage(page,{components:path.join(dir,'components.json'),name:stickScoreName(arm,run),familySet})
