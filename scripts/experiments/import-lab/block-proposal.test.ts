...
import { instrumentHtml, resolveAnchor, proposeGeometry, childCandidates, groupRows, proposeFromGeometry, TALL_BLOCK, type Geometry, type GeometryDocument } from './block-proposal'
const html='<!doctype html><body>text<!-- comment --><div id="wrapper"><section class="wide band"><h1>Example heading</h1></section></div></body>'
test('anchors use element indices, ignoring whitespace and comments',()=>{const result=instrumentHtml(html,'https://example.com/');expect(result.anchors['0.0']).toEqual({path:[0,0],tag:'section',id:'',classes:['wide','band']});expect(resolveAnchor(html,result.anchors['0.0'])).not.toBeNull();expect(result.html).toContain(result.marker)})
test('rejects changed identity, out-of-bounds and invalid indices',()=>{const anchor=instrumentHtml(html,'https://example.com/').anchors['0.0'];expect(resolveAnchor(html,{...anchor,tag:'article'})).toBeNull();expect(resolveAnchor(html,{...anchor,path:[8]})).toBeNull();expect(resolveAnchor(html,{...anchor,path:[-1]})).toBeNull()})
test('base preserves saved relative base resolved against final URL',()=>{expect(instrumentHtml('<head><base href="/assets/"></head><body></body>','https://example.com/page').baseUrl).toBe('https://example.com/assets/')})
...
function document(tree:Geometry):GeometryDocument {
  return {version:1,page:'fixture',snapshotSha256:'saved',finalUrl:'https://example.com/',javascriptEnabled:false,renderedHeight:tree.box.height,viewportWidth:1440,issues:[],tree}
}
const keys=(nodes:Geometry[])=>nodes.map(n=>n.key)
const text=(key:string,y:number,height:number)=>geometry(key,y,height,1170,[],{ownTextLength:100,ownContent:true})
...
test('single text article is kept and flagged probably under-cut without mutating its input',()=>{
  ...
  expect(result.issues).toContain('probably under-cut: block-dynamic-article is 3000px tall')
...
test('retains own wrapper text instead of silently dropping it',()=>{
  const root={...geometry('root',0,2000,1170,[text('a',0,1000),text('b',1000,1000)]),ownContent:true}
  expect(keys(proposeGeometry(root))).toEqual(['root'])
})

test('1500px is the warning boundary and only taller blocks are flagged',()=>{
  expect(proposeFromGeometry(document(text('article',0,1500))).issues.some(i=>i.startsWith('probably under-cut:'))).toBe(false)
  expect(proposeFromGeometry(document(text('article',0,1501))).issues.some(i=>i.startsWith('probably under-cut:'))).toBe(true)
})
...
test('instrumentation preserves the saved base elements without injecting one',()=>{
  expect(instrumentHtml(html,'https://example.com/').html).not.toContain('<base')
  const saved='<head><base href="/assets/"></head><body><p>Example</p></body>'
  expect(instrumentHtml(saved,'https://example.com/page').html.match(/<base /g)).toHaveLength(1)
})
...
function newsSection(y:number):Geometry {
  ...
  const articles=geometry('articles',y+60,740,1170,Array.from({length:4},(_,i)=>
    geometry('article'+i,y+60,740,270,[{...text('summary'+i,y+60,200),tag:'p'}],{tag:'article',box:{x:135+i*290,y:y+60,width:270,height:740}})))
...
function gridTiles(counts:number[],height=300):Geometry[] {
  return counts.flatMap((count,row)=>Array.from({length:count},(_,col)=>geometry('tile'+row+'-'+col,80+row*height,height,350,[],
    {ownTextLength:100,box:{x:135+col*380,y:80+row*height,width:350,height}})))
}
...
test('grid rows stop merging at 1500px and at heading-only separators',()=>{
  ...
  const tiles=gridTiles([3,3,3]);tiles.slice(3).forEach(n=>n.box.y+=60)
...
test.each(['position','width'])('grid column %s changes beyond five percent prevent a merge',change=>{
  ...
  tiles.slice(3,6).forEach(n=>{if(change==='position')n.box.x+=30;else n.box.width+=30})
  expect(proposeGeometry(geometry('grid',0,980,1170,tiles))).toHaveLength(3)
})
test('unstyled render metadata survives offline proposals',()=>{
  const result=proposeFromGeometry({...document(text('content',0,900)),styling:{declared:1,applied:0,inline:0,inaccessible:0},issues:['rendered without styling - blocks are not trustworthy']})
...
test('unstyled metadata adds the trust issue even when replay input omitted it',()=>{
  const result=proposeFromGeometry({...document(text('content',0,900)),styling:{declared:1,applied:0,inline:0,inaccessible:0}})
  expect(result.status).toBe('unstyled')
