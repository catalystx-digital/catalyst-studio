...
export function AIContextChat({ 
  ...
  const { data: contextData, isLoading: isLoadingContext } = useAIContext(websiteId, sessionId, { scope: 'website' });
  const createContext = useCreateAIContext();
  const appendMessage = useAppendMessage(websiteId, sessionId, 'website');
  const clearContext = useClearContext(websiteId, sessionId, 'website');
                ...
                >
                  ...
                  {message.metadata && (
                    <div className="mt-2 text-xs opacity-70">
                      {message.metadata.model ? `Model: ${String(message.metadata.model)}` : null}
                      {message.metadata.tokens ? ` • Tokens: ${String(message.metadata.tokens)}` : null}
                    </div>
                  )}
                </div>
              </div>
