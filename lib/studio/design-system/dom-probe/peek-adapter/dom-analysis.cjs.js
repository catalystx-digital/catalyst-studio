...
exports.__INTERNAL_ANALYSIS_IMPLEMENTATION = void 0;
exports.analyzeDomDocument = analyzeDomDocument;
exports.analyzeDom = analyzeDom;
const analysisImplementation = function analysisImplementation(documentRef, windowRef, options) {
    ...
    function computeRole(element, fontSizePx, weight, text) {
        ...
        if (!Number.isNaN(numericWeight) && numericWeight >= 600 && fontSizePx >= 18)
            return 'heading';
        if (fontSizePx >= 24)
            return 'heading';
        if (tag === 'label' || tag === 'small')
            return 'label';
        if (text.length <= 18 && tag === 'a')
            return 'label';
        return 'body';
    }
    function toTypographySampleId(sample) {
    ...
    function normalizeFontStack(value) {
        if (!value)
            return { fontFamily: 'Unknown', fontStack: 'Unknown' };
        const parts = value
            .split(',')
    ...
    const defaultView = windowRef ?? documentRef.defaultView;
    ...
    while (current && processed < settings.maxElements) {
        ...
        if (textContent.length >= settings.minTextLength) {
            ...
            if (fontSizePx && fontSizePx > 0) {
                ...
                const sample = typographyMap.get(key) ?? {
                    id: '',
                    selector,
                    fontFamily,
                    fontStack,
    ...
    function collectColor(element, style, map) {
        const selector = buildSelector(element);
    ...
    function collectSpacing(element, style, map) {
        const selector = buildSelector(element);
    ...
    function buildPalette(map, limit) {
        ...
        if (chromatic.length > 0) {
            const chromaticByUsage = [...chromatic].sort((a, b) => scoreChromatic(b) - scoreChromatic(a) || b.occurrences - a.occurrences);
    ...
    function scoreChromatic(swatch) {
    ...
    function harmonicSortSwatches(swatches) {
        ...
        const decomposeHueDistance = (a, b) => {
            const diff = Math.abs(a - b) % 360;
            return diff > 180 ? 360 - diff : diff;
        };
        const popAtIndex = (collection, index) => collection.splice(index, 1)[0];
        const lowChroma = [];
        const highChroma = [];
        ...
        while (highChroma.length) {
            ...
            sample.forEach((color, index) => {
                let score = LIGHTNESS_WEIGHT * Math.abs(previous._okl_l - color._okl_l) +
    ...
    function buildSpacing(map) {
        const tokens = Array.from(map.values())
            .sort((a, b) => {
            if (b.occurrences === a.occurrences)
                return a.valuePx - b.valuePx;
        ...
        if (tokens.length > 0) {
            ...
            if (primary) {
                ...
                if (secondary) {
                    ...
                    const secondaryDividesPrimary = primary.valuePx % secondary.valuePx === 0 || secondary.valuePx % primary.valuePx === 0;
    ...
    function detectMissingFonts(documentTarget, samples) {
        const fontStatus = new Set();
        const fonts = new Set();
        samples.forEach(sample => {
            sample.fontStack
                ...
                if (!font)
                    return;
                if (font.toLowerCase() === 'unknown')
                    return;
                if (GENERIC_FONTS.has(font.toLowerCase()))
                    return;
                fonts.add(font);
            });
        });
    ...
    function collectComponentCue(element, map) {
        if (map.size >= 40)
            return;
        const tag = element.tagName.toLowerCase();
        ...
        let inferredRole;
        let description;
        const classMatches = (patterns) => patterns.some(pattern => classList.some(cls => pattern.test(cls)));
        if (tag === 'button' || roleAttr === 'button' || classMatches([/btn/i, /button/i, /cta/i])) {
            ...
            const inputType = tag === 'input' ? (element.type || '').toLowerCase() : undefined;
...
exports.__INTERNAL_ANALYSIS_IMPLEMENTATION = analysisImplementation;
function analyzeDomDocument(documentRef, options) {
    const windowRef = documentRef.defaultView ?? null;
    return analysisImplementation(documentRef, windowRef, options);
}
function analyzeDom(options) {
    return analysisImplementation(document, window, options);
}
