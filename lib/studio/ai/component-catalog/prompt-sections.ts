...
export function renderPropertyNode(property: ComponentPropertyInfo, depth: number, lines: string[]): void {
  ...
  if (property.description) {
    notes.push(property.description)
  }
...
export const CONTENT_REFERENCE_RULES_SECTION = createStaticSection([
  ...
  '  Example: "cards": [{ "type": "card-item", "title": "...", "description": "...", "image": { "src": "...", "alt": "..." }, "link": "/offers", "linkText": "View offers", "metadata": { "category": "..." } }].',
...
export const CRITICAL_COMPLETENESS_SECTION = createStaticSection([
  ...
  '- hero-carousel: ALWAYS return a populated slides[] array with fully detailed slide objects as described. Missing or empty slides[] is incorrect when slides are visible.',
  '- card-grid: ALWAYS return a populated cards[] array with card-item/promo-item objects including titles, descriptions, media, links, and metadata pulled from the page.',
  ...
  '- hero-with-image: ALWAYS supply heading, copy, supporting media (image.src + alt), and CTA buttons when present. CTA buttons must use the "variant" field and include every visible action.',
  ...
  '- footer: ALWAYS populate columns[].links[], socialLinks[], and legalLinks[] with their documented child objects using stable ids and canonical link fields. Summaries or empty arrays are not acceptable when content is rendered.',
...
export const FORBIDDEN_FIELDS_SECTION = createStaticSection([
  ...
  '- "summary": NEVER emit on navigation components (navbar, footer, nav-menu-item, etc.). Only use summary when explicitly listed in the component contract.',
  '',
  ...
  '- footer.socialLinks[]: ONLY use type "socialLinkItem". NEVER use "nav-menu-item" for social links.',
  '- footer.legalLinks[]: Use type "nav-menu-item" with id/label/href/external fields.',
  '- footer.columns[]: Use type "columnItem" with nested links[] containing "nav-menu-item" children.',
  '',
